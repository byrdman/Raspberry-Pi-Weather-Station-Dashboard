...
var boolSoilTempProbePresent = true;
...
function setupData(result) {
    ...
    if (result.Settings.soilTemperatureProbePresent == "0") { boolSoilTempProbePresent = false; }
    ...
    [result.WeatherObservations.Observation1,result.DailyStats].forEach(function (obj) { 
        for (var property in obj) {
            if (obj.hasOwnProperty(property)) {
                rawList += "<li>" + property + ": " + obj[property] + "</li>";
            }
        }
    });
    rawList += "</ul>";
    $("#rawData").append(rawList);
}
